...
test("names the install page from host slug", () => {
  assert.equal(appNameFromHost("localhost:8080"), "XP App");
  assert.equal(appNameFromHost("172.17.154.217:8080"), "XP App");
  assert.equal(appNameFromHost("wild-race.grok.me"), "Wild Race");
});

test("rejects hosts that are not plain slugs", () => {
  assert.equal(appNameFromHost("<script>alert(1)</script>"), "XP App");
  assert.equal(appNameFromHost('"><img src=x onerror=1>.grok.me'), "XP App");
});
