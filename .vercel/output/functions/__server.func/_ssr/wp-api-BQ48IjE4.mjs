import { At as number, Pt as string, jt as object } from "../_libs/@better-auth/core+[...].mjs";
import { n as createServerFn } from "./ssr.mjs";
import { a as listings, r as guides, s as parseOpenHoursHtml } from "./hours-BdXwVY0q.mjs";
import { a as uncropImage, i as pickListingImage, o as uniqueImages, t as extractOgImage } from "./media-Cl4fnsUk.mjs";
import { t as createServerRpc } from "./createServerRpc-CN-evIEF.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/wp-api-BQ48IjE4.js
...
function mergeHit(prev, hit) {
	...
	return {
		...
		listingPackage: prev.listingPackage != null && hit.listingPackage != null ? Math.min(prev.listingPackage, hit.listingPackage) : prev.listingPackage ?? hit.listingPackage
	};
}
...
async function loadJetArchiveMeta(categorySlugs, into) {
	const bySlug = into ?? /* @__PURE__ */ new Map();
	const markers = /* @__PURE__ */ new Map();
	await pool([...new Set(categorySlugs.filter(Boolean))].slice(0, 8), 4, async (cat) => {
...
function urlTail$1(url) {
	return url.split("/").filter(Boolean).pop() ?? "";
}
function decode(raw) {
	return raw.replace(/&/gi, "&").replace(/&#038;/g, "&").replace(/"/gi, "\"").replace(/&#(\d+);/g, (_, n) => String.fromCharCode(Number(n))).replace(/\s+/g, " ").trim();
}
function categoryFromCard(kind, listingType) {
...
function parseListeoGeoHtml(html) {
	const out = [];
	const seen = /* @__PURE__ */ new Set();
	const chunks = html.split(/listing-geo-data/);
	for (const chunk of chunks.slice(1)) {
		const window = chunk.slice(0, 9e3);
		...
		const slug = href ? urlTail$1(href[0].replace(/\/$/, "")) : "";
		if (!slug || seen.has(slug)) continue;
		seen.add(slug);
		...
		const address = decode(window.match(/data-friendly-address="([^"]*)"/)?.[1] ?? window.match(/data-address="([^"]*)"/)?.[1] ?? "");
		const poster = window.match(/data-image="([^"]+)"/)?.[1];
		const slides = [...window.matchAll(/src="(https:\/\/xplorepondy\.com\/wp-content\/uploads\/[^"]+)"/g)].map((m) => m[1]);
		...
		out.push({
			...
			reviews: Number.isFinite(reviews) && reviews > 0 ? reviews : void 0,
...
async function fetchListeoPage(page, perPage) {
...
async function loadListeoGeo() {
	...
	for (const html of pages) for (const hit of parseListeoGeoHtml(html)) if (!map.has(hit.slug)) map.set(hit.slug, hit);
	return map;
}
...
function extractGallery(html, featured) {
	...
	].flatMap((block) => [...block.matchAll(/(?:src|data-src|href)="(https:\/\/xplorepondy\.com\/wp-content\/uploads\/[^"]+\.(?:jpe?g|png|webp))"/gi)].map((m) => m[1]));
...
async function loadCatalogFromWp() {
	const jetMeta = /* @__PURE__ */ new Map();
	const jetTask = loadJetArchiveMeta(ARCHIVE_SLUGS, jetMeta).catch(() => jetMeta);
	const listeoGeo = await loadListeoGeo().catch(() => /* @__PURE__ */ new Map());
	await Promise.race([jetTask, new Promise((resolve) => setTimeout(resolve, 1e4))]);
	if (listeoGeo.size < 8 && jetMeta.size < 8) throw new Error("listeo-empty");
	const seen = /* @__PURE__ */ new Set();
	const listings = [];
	function applyLive(item, slug) {
	...
	return {
		listings: merged,
		total: Math.max(merged.length, listeoGeo.size, jetMeta.size)
	};
}
...
var fetchWpListing = createServerFn({ method: "GET" }).validator(object({
	slug: string().min(1),
	url: string().optional()
})).handler(fetchWpListing_createServerFn_handler, async ({ data }) => {
	const res = await wpGet(`/wp-json/wp/v2/listing?slug=${encodeURIComponent(data.slug)}&_embed=1`, {}, 15e3);
	let listing = null;
	if (res.ok && Array.isArray(res.data) && res.data[0]) listing = mapListing(res.data[0]);
	const local = findLocal(data.slug, data.url);
	...
	for (const url of urls) try {
		const page = await fetch(url, {
			headers: {
				Accept: "text/html",
				"User-Agent": "Mozilla/5.0 XplorePondyApp/1.0"
			},
		...
		return enrichListingFromHtml({
			...base,
			siteUrl: base.siteUrl || url
		}, html);
	} catch {}
	return seed;
});
var fetchWpUserTrips_createServerFn_handler = createServerRpc({
	id: "8269a8f231a65c8a73273d0e7bc15b46a8d2034c81c87ff910a86e7758a28375",
	name: "fetchWpUserTrips",
	filename: "src/lib/wp-api.ts"
}, (opts) => fetchWpUserTrips.__executeServer(opts));
var fetchWpUserTrips = createServerFn({ method: "GET" }).handler(fetchWpUserTrips_createServerFn_handler, async () => {
	const res = await wpGet("/wp-json/wp/v2/user_trip?per_page=12&orderby=date&order=desc&_fields=id,slug,title,date,link", {}, 12e3);
	if (!res.ok || !Array.isArray(res.data)) return { trips: [] };
	return { trips: res.data.map((t) => ({
		id: t.id,
		slug: t.slug,
...
//#endregion
export { fetchWpAuthorContent_createServerFn_handler, fetchWpCatalog_createServerFn_handler, fetchWpGuide_createServerFn_handler, fetchWpGuides_createServerFn_handler, fetchWpListing_createServerFn_handler, fetchWpUserTrips_createServerFn_handler, wpLogin_createServerFn_handler };
